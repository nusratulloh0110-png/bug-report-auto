...
Для маппинга продукта в Jira Project Key сервис использует лист `Ключ Jira` в Google Sheets. Там можно задать связку вида `ЛИС -> LIS`, `Склад -> CORE`, `Касса -> CASH`, не меняя код бота.

В Jira отправляются:

- проект по `JIRA_PROJECT_KEY`
- тип задачи выбирается в модальном окне из списка типов выбранного проекта Jira
- `JIRA_ISSUE_TYPE_NAME` и `JIRA_ISSUE_TYPE_ID` используются как значение по умолчанию, если такой тип есть в выбранном проекте
- summary
...
Invoke-WebRequest -Method POST `
  ...
  http://127.0.0.1:3000/internal/publish-launcher
```
